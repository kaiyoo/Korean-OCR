...
def main(config, device, logger, vdl_writer, seed):
    ...
    for name, layer in model.named_sublayers():
        if isinstance(layer, paddle.nn.GRU) or isinstance(layer, paddle.nn.LSTM):
            layer.flatten_parameters()

    # with open("./model_param_names.txt", "w") as f:
    #     for name, param in model.named_parameters():
    #         f.write(name + "\n")
    # print("모델 파라미터 이름을 model_param_names.txt 에 저장했습니다.")
    ...
    logger.info("if os.path.exists Global save_model_")
    ...
    # last iter 정보 이어학습하고 싶을때:
    # pretrained_model_path = config["Global"].get("pretrained_model", "")
    # states_path = pretrained_model_path.replace(".pdparams", ".states") if pretrained_model_path.endswith(".pdparams") else None

    # if states_path and os.path.exists(states_path):        
    #     with open(states_path, "rb") as f:
    #         states_dict = pickle.load(f, encoding="latin1")
    #     pre_best_model_dict["start_epoch"] = states_dict.get("epoch", 0) + 1
    #     pre_best_model_dict["global_step"] = states_dict.get("global_step", 0)
    #     logger.info(f"✅ Resuming from epoch {pre_best_model_dict['start_epoch']}, step {pre_best_model_dict['global_step']}")
    # else:
    #     pre_best_model_dict["start_epoch"] = 1
    #     pre_best_model_dict["global_step"] = 0
...
if __name__ == "__main__":
    ...
    #     logger.info(f"Epoch {epoch+1} / {config['Global']['epoch_num']}")
    #     # Dummy training
    #     for i in range(5):
    #         logger.info(f"Step {i+1}/5")
