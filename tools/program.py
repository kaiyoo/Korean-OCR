...
def train(
    ...
    # )
    start_epoch = pre_best_model_dict.get("start_epoch", 1)
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        #     # batch_size 줄이기 위해 Dataloader만 재생성
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            #     break
            # end_time = time.time()
            # print(f"▶ Dataloader 10 step 로딩 시간: {end_time - start_time:.2f}초")
            # print(f"▶ 평균 1 step 당 로딩 시간: {(end_time - start_time) / 10:.2f}초")
            ...
            if (
                ...
                if cur_metric[main_indicator] >= best_model_dict[main_indicator]:
                    ...
                    logger.info(f"✅ Best model saved at epoch {epoch}, step {global_step}, val acc: {best_model_dict['acc']}, lr: {float(optimizer.get_lr())}")
...
def eval(
    ...
    with paddle.no_grad():
        ...
        for idx, batch in enumerate(valid_dataloader):
            ...
            # Prediction sample 출력
            if sample_count < 5:
                sample_batch_index=0
                pred_text, pred_score = post_result[0][sample_batch_index]
                gt_text, _ = post_result[1][sample_batch_index]
                logger.info(f"GT: {gt_text}, Prediction: {pred_text}, Confidence: {pred_score:.4f}")
                sample_count += 1
...
def preprocess(is_train=False):
    ...
    # log_ranks = config["Global"].get("log_ranks", 0 ) #"0")
